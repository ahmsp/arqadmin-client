 ...
 * logged out.
 */
Ext.define('ArqAdmin.util.OAuthMonitor', {
    singleton: true,

    doRefreshTokenInterval: 1000 * 60 * 50, // 50 minutes to do refresh token
    startTime: null,

    lastActive: null,
    maxInactive: 1000 * 60 * 120, // 2 hours of inactivity allowed
    remaining: 0,
        ...
        items: [
            {
                ...
                frame: true,
                html: [
                    'Sua sessão expira automaticamente após 2 horas de inatividade. ',
                    'Se a sessão expirar, os dados não salvos serão perdidos e a aplicação será automaticamente encerrada.',
                    '<br><br>',
        ...
        buttons: [
            {
                ...
                handler: function () {
                    Ext.TaskManager.stop(ArqAdmin.util.OAuthMonitor.countDownTask);
                    ArqAdmin.util.OAuthMonitor.window.hide();
    ...
    captureActivity: function (eventObj, el, eventOptions) {
        ...
        // if (elapsedTime >= this.doRefreshTokenInterval) {
        //     this.stop();
        //     ArqAdmin.app.getController('OAuth').doRefreshToken();
        // }
    },
     ...
     *  or the time limit for the token
     */
    monitorUI: function () {
        var now = new Date();
